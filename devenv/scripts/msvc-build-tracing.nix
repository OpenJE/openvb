# ./devenv/scripts/msvc-build-tracing.nix
...
in {
  scripts.msvc-build-tracing = {
    description = "Build OpenVB with tracing";
    exec = /* bash */ ''
      set -euo pipefail
      ...
      "$docker" run --rm -t \
        ...
        -lc '
          WINEPREFIX=/work/build/wineprefix \
            wine cmd /c "call Z:\\home\\msvc2002\\bin\\vcvars32.bat >NUL && %CMAKE_EXE_WIN% -S Z:\\work -B Z:\\work\\build-msvc-tracing -DCMAKE_BUILD_TYPE=Release -DOPENVB_TRACING=ON -DCMAKE_TRY_COMPILE_TARGET_TYPE=STATIC_LIBRARY -DCMKR_SKIP_GENERATION=ON"
          WINEPREFIX=/work/build/wineprefix \
